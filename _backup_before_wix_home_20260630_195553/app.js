  ...
  $('#app').innerHTML = `
    <header class="topbar">
      <nav class="nav">
        <div class="brand">
          ...
          <div><div class="brand-title">${esc(c.siteName)}</div><div class="brand-sub">Sistema editÃ¡vel</div></div>
        </div>
        <div class="menu">
    ...
    <main>
      ...
      <section class="section" id="produtos">
        <div class="section-head"><div><h2>Produtos</h2><p class="section-intro">Categorias e cards editÃ¡veis pelo painel. Cada Saiba Mais abre popup prÃ³prio, sem dependÃªncia do Wix.</p></div></div>
      ...
      <section class="section" id="institucional">
        <div class="section-head"><div><h2>Institucional</h2><p class="section-intro">Blocos editÃ¡veis. O campo HTML aceita marcaÃ§Ã£o simples sanitizada.</p></div></div>
        <div class="content-blocks">${db.blocks.filter(b=>b.enabled).sort((a,b)=>a.order-b.order).map(renderBlock).join('')}</div>
      </section>
      <section class="section" id="galeria">
        <div class="section-head"><div><h2>Galeria base</h2><p class="section-intro">Imagens do pacote inicial preservadas como referÃªncia visual.</p></div></div>
    ...
    <div class="modal-backdrop" id="modal"><div class="modal"><button class="modal-close" onclick="closeModal()">Ã—</button><div class="modal-content" id="modalContent"></div></div></div>
...
function openRepresentatives(){
  $('#modalContent').innerHTML = `<h3>Representantes</h3><p>Lista editÃ¡vel no mock database.</p><div class="edit-list">${db.representatives.map(r=>`<div class="edit-item"><strong>${esc(r.state)}</strong><br>${esc(r.name)}<br>${esc(r.phone)}<br>${esc(r.email)}</div>`).join('')}</div>`;
  $('#modal').classList.add('open');
}
...
function addProduct(){ const title='Novo Produto'; db.products.push({id:'novo-produto-'+Date.now(),category:db.categories[0]?.title||'Produtos',title,subtitle:'',image:'assets/screenshots/021_1782839081.png',shortDescription:'Resumo do produto.',details:['Detalhe tÃ©cnico editÃ¡vel.'],visible:true,order:db.products.length+1}); save(); render(); toggleAdmin(true); }
...
function deleteCategory(id){ if(confirm('Excluir categoria? Produtos nÃ£o serÃ£o apagados.')){db.categories=db.categories.filter(c=>c.id!==id); save(); render(); toggleAdmin(true);} }
...
function importJson(){ try{ db=JSON.parse($('#importJson').value); save(); render(); toggleAdmin(true); }catch(e){ alert('JSON invÃ¡lido'); } }
...
/* PILLER_PATCH_START */
function pillerNormalizeDb(){
  if(!db) return;

  db.config = db.config || {};
  db.config.siteName = db.config.siteName || "Piller Metal";
  db.config.headline = db.config.headline || "Fechaduras e cadeados Piller Metal";
  db.config.subheadline = db.config.subheadline || "Catálogo editável com produtos, representantes, galerias e conteúdo institucional.";
  db.config.primaryColor = db.config.primaryColor || "#072b61";
  db.config.secondaryColor = db.config.secondaryColor || "#f5b51b";
  db.config.accentColor = db.config.accentColor || "#0d4f9a";
  db.config.footerText = db.config.footerText || "Piller Metal";
  ...
  db.buttons = Array.isArray(db.buttons) ? db.buttons : [];
  db.representatives = Array.isArray(db.representatives) ? db.representatives : [];

  if(!db.categories.length){
    db.categories = [
      {id:"cadeados",title:"Cadeados",visible:true,order:1},
      {id:"cilindros",title:"Cilindros",visible:true,order:2},
      {id:"fechaduras",title:"Fechaduras",visible:true,order:3}
    ];
  }

  db.products.forEach((p,i)=>{
    p.id = p.id || slug((p.title || "produto") + "-" + i);
    p.title = p.title || "Produto Piller";
    p.category = p.category || db.categories[0].title;
    p.subtitle = p.subtitle || "";
    p.shortDescription = p.shortDescription || "Produto configurável pelo painel administrativo.";
    p.details = Array.isArray(p.details) ? p.details : String(p.details || "").split("\n").filter(Boolean);
    p.visible = p.visible !== false;
    p.order = Number(p.order || i + 1);
    p.image = p.image || "assets/screenshots/021_1782839081.png";
  });
    ...
    db.blocks = [
      {
        ...
        title:"Institucional",
        type:"text",
        content:"A Piller Metal atua com soluções em fechaduras, cadeados, cilindros e acessórios, mantendo foco em segurança, qualidade e apresentação técnica dos produtos.",
        enabled:true,
        order:1
      }
    ];
  ...
  if(!db.buttons.length){
    db.buttons = [
      {id:"btn-produtos",label:"Ver produtos",targetType:"internal",target:"#produtos",x:42,y:76,width:170,enabled:true,order:1},
      {id:"btn-representantes",label:"Representantes",targetType:"popup",target:"representantes",x:58,y:76,width:180,enabled:true,order:2}
    ];
  }
}
...
function renderPublic(){
  ...
  const c = db.config;
  const categories = db.categories.filter(x=>x.visible).sort((a,b)=>Number(a.order||0)-Number(b.order||0));
  const products = db.products
    .filter(p=>p.visible && (!activeCategory || activeCategory==='Todos' || p.category===activeCategory))
  ...
  $('#app').innerHTML = `
    <header class="topbar">
      <nav class="nav">
        <div class="brand">
          <img class="brand-mark" src="${esc(c.logoImage || '')}" onerror="this.style.display='none'" />
          <div>
            <div class="brand-title">${esc(c.siteName)}</div>
            <div class="brand-sub">Catálogo editável</div>
          </div>
        </div>

        <div class="menu" aria-label="Menu principal">
          <a href="#inicio">INÍCIO</a>
          <a href="#institucional">INSTITUCIONAL</a>
          <a href="#produtos">PRODUTOS</a>
          <button type="button" onclick="openRepresentatives()">REPRESENTANTES</button>
          <a href="#galeria">GALERIA</a>
          <button type="button" class="admin-toggle" onclick="toggleAdmin(true)">ADMIN</button>
        </div>
      </nav>
    </header>
    ...
    <main>
      <section class="hero" id="inicio">
        <div class="hero-bg" style="background-image:url('${esc(c.backgroundImage || 'assets/screenshots/021_1782839081.png')}')"></div>
        <div class="hero-inner">
          <div class="eyebrow">Piller Metal</div>
          <h1>${esc(c.headline)}</h1>
          <p>${esc(c.subheadline)}</p>
      ...
      <section class="section" id="produtos">
        <div class="section-head">
          <div>
            <h2>Produtos</h2>
            <p class="section-intro">Categorias, ordem, textos, imagens e popups editáveis. Cada botão Saiba Mais abre um modal próprio.</p>
          </div>
        </div>

        <div class="category-tabs">
          <button type="button" class="tab ${activeCategory==='Todos'?'active':''}" onclick="setCategory('Todos')">Todos</button>
          ${categories.map(cat=>`<button type="button" class="tab ${activeCategory===cat.title?'active':''}" onclick="setCategory('${esc(cat.title)}')">${esc(cat.title)}</button>`).join('')}
        </div>
      ...
      <section class="section" id="institucional">
        <div class="section-head">
          <div>
            <h2>Institucional</h2>
            <p class="section-intro">Textos e blocos HTML configuráveis no painel.</p>
          </div>
        </div>
        <div class="content-blocks">
          ${db.blocks.filter(b=>b.enabled).sort((a,b)=>Number(a.order||0)-Number(b.order||0)).map(renderBlock).join('')}
        </div>
      </section>
      ...
      <section class="section" id="galeria">
        <div class="section-head">
          <div>
            <h2>Galeria</h2>
            <p class="section-intro">Imagens preservadas do pacote original para referência visual e reaproveitamento.</p>
          </div>
        </div>
        <div class="gallery">
          ${Array.from({length:48},(_,i)=>`<img src="assets/screenshots/${String(i).padStart(3,'0')}_${screenshotSuffix(i)}.png" loading="lazy" onerror="this.remove()"/>`).join('')}
        </div>
      </section>
    </main>
...
function renderProductCard(p){
  ...
  const subtitle = p.subtitle && p.subtitle !== category ? `<div class="card-kicker">${esc(p.subtitle)}</div>` : "";
  return `
    <article class="card">
      <div class="card-img">
        <img src="${esc(p.image)}" loading="lazy" onerror="this.src='assets/screenshots/021_1782839081.png'" />
      </div>
      <div class="card-body">
        <div class="card-kicker">${esc(category)}</div>
        <h3>${esc(p.title)}</h3>
        ${subtitle}
        <p>${esc(p.shortDescription || '')}</p>
        <div class="card-actions">
          <button type="button" class="btn" onclick="openProduct('${esc(p.id)}')">Saiba Mais</button>
          <button type="button" class="btn ghost" onclick="focusAdminProduct('${esc(p.id)}')">Editar</button>
        </div>
      </div>
    </article>
  `;
...
function openProduct(id){
  ...
  const extraHtml = p.html ? `<div class="block">${sanitizeHtml(p.html)}</div>` : "";

  $('#modalContent').innerHTML = `
    <div class="modal-product">
      <img src="${esc(p.image)}" onerror="this.src='assets/screenshots/021_1782839081.png'" />
      <div>
        <div class="card-kicker">${esc(p.category || '')}</div>
        <h3>${esc(p.title)}</h3>
        ${p.subtitle ? `<p><strong>${esc(p.subtitle)}</strong></p>` : ""}
        <p>${esc(p.shortDescription || '')}</p>
        ${details.length ? `<ul>${details.map(d=>`<li>${esc(d)}</li>`).join('')}</ul>` : ""}
        ${extraHtml}
        <div class="card-actions">
          <button type="button" class="btn" onclick="closeModal()">Fechar</button>
          <button type="button" class="btn ghost" onclick="focusAdminProduct('${esc(p.id)}')">Editar produto</button>
        </div>
      </div>
    </div>
  `;
...
function openRepresentatives(){
  ...
  const reps = db.representatives.length ? db.representatives : [
    {state:"RJ",name:"Representante Piller",phone:"",email:""}
  ];

  $('#modalContent').innerHTML = `
    <h3 style="color:var(--primary);font-size:34px;margin-top:0">Representantes</h3>
    <p class="section-intro">Lista configurável no mock database.</p>
    <div class="rep-grid">
      ${reps.map(r=>`
        <div class="rep-card">
          <strong>${esc(r.state || r.region || 'Região')}</strong><br>
          ${esc(r.name || '')}<br>
          ${r.phone ? `<span>${esc(r.phone)}</span><br>` : ""}
...
setTimeout(()=>{
  try{
    ...
    console.error("Erro no patch Piller:", err);
  }
}, 150);
/* PILLER_PATCH_END */
init();
